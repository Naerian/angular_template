...
import { RadioButtonsOrientation, NEO_RADIO_BUTTON_GROUP } from '../models/radio-button.model';
...
export class RadioButtonGroupComponent implements ControlValueAccessor {
  ...
  @Input() direction: RadioButtonsOrientation = 'horizontal';
  ...
  ngAfterViewInit(): void {
    ...
    if (
      ...
      this._name.set(
        this._inputsUtilsService.createUniqueId('radiobutton-group'),
      );
      this.updateRadioButtonsName();
    }
  }
  ...
  updateInputSizeRadioButtons(): void {
    ...
    this.radioButtons.forEach(
      (radioButton) => (radioButton.inputSize = this._inputSize || 'm'),
    );
  }
  ...
  updateRadioButtonsName(): void {
    if (!this.radioButtons) return;
    this.radioButtons.forEach(
      (radioButton) => (radioButton.name = this._name()),
    );
  }
  ...
  disableRadioButtons(): void {
    if (!this.radioButtons) return;
    this.radioButtons.forEach(
      (radioButton) => (radioButton.disabled = this._disabled()),
    );
  }
